...
services:
  ...
  matrix-pg:
    ...
    volumes:
        # 持久化数据放置目录
      - ${DATA_PATH:?data path required}/data/postgres:/var/lib/postgresql/data
        # 脚本目录
...
- DATA_PATH：Postgres 数据库的数据存放地址（持久化数据）
...
```
cd && mkdir -p /data/synapse/init && cd /data/synapse/init && touch "matrix.sql"
```
